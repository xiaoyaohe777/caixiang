...
class ZodiacCalculator:
    def __init__(self, root):
        self.root = root
        self.root.title("采真属相排盘工具1-70逐年")
    ...
    def create_widgets(self):
        ...
        # 计算按钮
        ttk.Button(input_frame, text="计算70年采相分值", command=self.calculate).grid(row=1, column=7, padx=10)
    ...
    def calculate(self):
        try:
            ...
            # 计算并显示未来70年的评分
            self.result_text.insert(tk.END, "=== 70年采相分值 ===\n\n", 'big')
            
            years_to_calc = year + 1
            ...
            for _ in range(70):          
                ...
                animals = [self.nian, yue, self.ri, shi]
                results = self.calculate_scores(animals)
                
                # 显示每年的评分结果，第一行使用大字体
                self.result_text.insert(tk.END, f"{years_to_calc}年{month}月:{self.nian} {yue} {self.ri} {shi}\n", 'title')
